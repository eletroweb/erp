<template>
    <div class="q-pa-md">
        <q-table id="financeiro" virtual-scroll style="height: 700px" :rows-per-page-options="[12]" :filter="filter"
            :rows="financeiroStore.financeiro" :columns="columns" flat no-data-label="Não existem financeiro lançadas"
            no-results-label="Nenhum registro localizado" bordered title="Financeiro" row-key="name">

            <template v-slot:top-right>

                <q-btn size="md" color="primary" label="Cadastrar Financeiro" no-caps @click="financeiroStore.novo()" />
