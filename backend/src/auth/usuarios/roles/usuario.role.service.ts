...
export class UsuarioRoleService {
    ...
    constructor(
        @InjectRepository(UsuarioRoleEntity) private usuarioRoleRepository: Repository<UsuarioRoleEntity>,
    ...
    async adicionarRoleAoUsuario(usuario: UsuarioEntity, requestRoles: string[]): Promise<void> {
        try {
            ...
            this.logger.debug(`Erro ao tentar vincular roles ao usuário: ${error.message}`);
        }
    }

    private async removerTodasRoles(usuarioId: number): Promise<void> {
        await this.usuarioRoleRepository.createQueryBuilder()
            .delete()
            .where("usuarioId = :usuarioId", { usuarioId })
            .execute();
    }

    private mapUsuarioRoles(usuario: UsuarioEntity, roles: RoleEntity[]): UsuarioRoleEntity[] {
        if (usuario.roles) {
    ...
    private async removerRolesNaoExistentes(usuario: UsuarioEntity, roles: RoleEntity[]): Promise<void> {
    ...
    private filtrarERemoverRolesDuplicadas(usuario: UsuarioEntity, roles: RoleEntity[]): UsuarioRoleEntity[] {
        return roles
            .filter(role => !usuario.roles.some(ur => ur.roles.id === role.id))
            .map(role => this.criarUsuarioRole(role, usuario));
    }

    private criarUsuarioRoles(roles: RoleEntity[], usuario: UsuarioEntity): UsuarioRoleEntity[] {
        return roles.map(role => this.criarUsuarioRole(role, usuario));
    }

    private criarUsuarioRole(role: RoleEntity, usuario: UsuarioEntity): UsuarioRoleEntity {
    ...
    async updateRoleByUserUuid(usuario: UsuarioEntity, roles: string[]): Promise<any> {
        await this.adicionarRoleAoUsuario(usuario, roles)
        return true
    }

    async getRolesByUserUuid(usuario: UsuarioEntity): Promise<UsuarioRoleEntity[]> {
        return await this.usuarioRoleRepository.find({
            where: { usuario: usuario }, relations: ['roles'],
            select: {
                roles: {
                    uuid: true,
                    nome: true,
