...
export class UsuarioController {
  ...
  @Put(':uuid')
  @Roles({ roles: ['MASTER', 'EDITAR_USUARIO'] })
  async update(@Param('uuid', new ParseUUIDPipe({ version: '4' })) uuid: string, @Body() request: UsuarioRequestDto): Promise<string> {
  ...
  @Put('roles/:uuid')
  @Roles({ roles: ['MASTER', 'EDITAR_USUARIO'] })
  async updateRoles(@Param('uuid', new ParseUUIDPipe({ version: '4' })) uuid: string, @Body() request: string[]): Promise<string> {
    const usuario = await this.usuarioService.findOneByUuid(uuid);
  ...
  @Roles({ roles: ['MASTER', 'EDITAR_USUARIO'] })
  async getRolesByUserUuid(@Param('uuid', new ParseUUIDPipe({ version: '4' })) uuid: string): Promise<UsuarioRoleResponseDto[]> {
    const usuario = await this.usuarioService.findOneByUuid(uuid);
    const usuariosEntityList = await this.usuarioRoleService.getRolesByUserUuid(usuario)
    const response: UsuarioRoleResponseDto[] = usuariosEntityList.map(role => {
