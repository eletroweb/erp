    ...
    async vencida(parcelas: FinanceiroParcelasEntity[]): Promise<boolean> {
        if (parcelas != undefined) {
            return parcelas.some(parcela => {
                return dayjs(parcela.data_vencimento).startOf('day').isBefore(dayjs().startOf('day')) && [FinanceiroEnum.PENDENTE].includes(parcela.situacao);
            });
        }
        return false
    }

    async parscelasVencidas(parcelas: FinanceiroParcelasEntity[]): Promise<FinanceiroParcelasEntity[]> {
        if (parcelas != undefined) {
