...
export class FinanceiroParcelaService {
    ...
    async todasParcelasPagas(parcelas: FinanceiroParcelasEntity[]): Promise<boolean> {
        return parcelas.every(parcela => parcela.situacao === FinanceiroEnum.PAGA);
    }

    async adicionarParcelaNaFinanceiro(
        financeiro: FinanceiroEntity, parcelas: FinanceiroParcelaRequest[]
            ...
            await this.financeiroParcelaRepository
                .createQueryBuilder()
                .delete()
                .where("financeiroId = :financeiroId AND parcela NOT IN (:...requestParcelIds)",
            ...
            const newParcelas = parcelas.map(financeiro =>
                FinanceiroParcelasEntity.toEntity(financeiro)
            );
