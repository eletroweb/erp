...
export class ContratoController {
  ...
  async findAll(): Promise<ContratoResponseDto[]> {
    ...
    const contratosDto: ContratoResponseDto[] = contratos.map((contrato) =>
      contrato.toDto(),
    );
    return contratosDto;
  }
  ...
  async findOne(@Param('uuid') uuid: string): Promise<ContratoResponseDto> {
    ...
    if (!contrato) throw new NotFoundException('Contrato não localizado');
  ...
  async update(
    ...
    @Body() contratoEntity: ContratoEntity,
  ): Promise<string> {
    const updatedContrato = await this.contratoService.update(
      uuid,
      contratoEntity,
    );
