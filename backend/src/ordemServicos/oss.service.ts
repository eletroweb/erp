...
export class OssService {
  ...
  async createMany(ordem_servico: OrdemServicoEntity, items: OssRequestDto[]): Promise<void> {
    ...
    await Promise.all(await this.removerOssResidual(promises, existingOssEntities, items));
  }

  async removerOssResidual(promises: Promise<OssEntity>[], existingOssEntities: OssEntity[], items: OssRequestDto[]): Promise<Promise<OssEntity>[]> {
