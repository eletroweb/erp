...
export class OrdemServicoController {
  ...
  async findAll(): Promise<OrdemServicoResponseDto[]> {
    ...
    const ordemServicosDto: OrdemServicoResponseDto[] = ordemServicos.map(
      (ordemServico) => ordemServico.toDto(),
    );
  ...
  async update(
    ...
    @Body() request: OrdemServicoRequestDto,
  ): Promise<string> {
    const updatedOrdemServico = await this.ordemServicoService.update(
      uuid,
      request,
    );
